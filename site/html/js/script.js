window.addEventListener("message", async (e) => {
    ...
    for (let el in esls) {
        ...
        for (let i = 0; i < esls[el].length; i += 2) {
            ...
            table.appendChild(createRow([es[i], es[i + 1]],e.data.folderID));
        }
...
function createRow(esls,folderID) {
    let tr = document.createElement("tr");
    for (let e of esls) {
        if (e == null) continue;
